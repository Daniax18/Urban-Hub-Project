...
class MockLogConsumer(LogConsumerPort):
    """Mock consumer pour tests"""

    def __init__(self):
        """Initialise le mock consumer"""
        self._is_connected = False
        self._callback: Optional[Callable] = None

    def start(self, callback: Callable[[str], None]) -> None:
        """Démarre le mock consumer"""
        self._is_connected = True
        self._callback = callback

    def stop(self) -> None:
        """Arrête le mock consumer"""
        self._is_connected = False

    def is_connected(self) -> bool:
        """Vérifie si le mock consumer est connecté"""
        return self._is_connected

    def publish_message(self, message: str) -> None:
        """Publie un message pour test"""
        if self._callback:
